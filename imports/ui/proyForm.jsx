...
class PForm extends React.Component {
    ...
    addProject(evt) {
        ...
        if (this.checkGithub() === 'success') {
            this.props.addProject({'url': this.state.url, 'description': this.state.description, 'collaborator': this.state.collaborator});
            this.setState({url: '', description: '', collaborator: '', goodAlert: true});
        } else {
